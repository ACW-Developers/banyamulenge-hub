      ...
      <div className="rounded-2xl border bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-sm">{t("home.adverts.title")}</h3>
              <p className="text-[11px] text-gray-500">{t("home.adverts.subtitle")}</p>
            </div>
          </div>
        ...
        {isLoading ? (
          ...
          <p className="text-xs text-gray-500 text-center py-6">{t("home.adverts.empty")}</p>
        )}
      </div>
    </aside>
  );
...
function AdvertCard({ advert }: { advert: Advert }) {
  ...
  const del = useMutation({
    ...
    onSuccess: () => {
      toast.success(t("home.adverts.removed"));
      qc.invalidateQueries({ queryKey: key });
    },
      ...
      <div className="p-3">
        <div className="flex items-start justify-between gap-2">
          ...
          {isAdmin && (
            <button
              ...
              aria-label={t("home.adverts.delete")}
            >
        ...
        <div className="mt-2 flex items-center justify-between">
          ...
          {advert.link_url && (
            ...
            >
              {t("home.adverts.learnMore")} <ExternalLink className="h-3 w-3" />
            </a>
          )}
        </div>
      </div>
...
function NewAdvertDialog() {
  ...
  function pick(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (f.size > 5 * 1024 * 1024) {
      toast.error(t("home.adverts.imageTooBig"));
      return;
    }
    setFile(f);
  ...
  async function submit() {
    ...
    try {
      ...
      if (error) throw error;
      toast.success(t("home.adverts.published"));
      setOpen(false);
      setTitle("");
      setContent("");
      setLink("");
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="sm" variant="outline" className="gap-1 h-8 px-2">
          <Plus className="h-3.5 w-3.5" /> {t("home.adverts.new")}
        </Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t("home.adverts.newPostTitle")}</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <div className="space-y-1.5">
            <Label>{t("home.adverts.titleLabel")}</Label>
            <Input
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              placeholder={t("home.adverts.titlePlaceholder")}
            />
          </div>
          <div className="space-y-1.5">
            <Label>{t("home.adverts.contentLabel")}</Label>
          ...
          <div className="space-y-1.5">
            ...
            <label className="cursor-pointer inline-flex items-center gap-2 px-3 py-2 rounded-lg border text-sm text-gray-600 hover:bg-gray-50 w-fit">
              <ImageIcon className="h-4 w-4" /> {t("home.adverts.chooseImage")}
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => setOpen(false)}>
            {t("home.adverts.cancel")}
          </Button>
          <Button onClick={submit} disabled={busy || !title.trim() || !content.trim()}>
            {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : t("home.adverts.publish")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
